...
class DefaultNetworkRequestProcessor: NetworkRequestProcessor {
    ...
    private let errorBodyConverter: ErrorBodyConverter
    
    init(sessionProvider: SessionProvider, tokenRefresher: TokenRefresher, errorBodyConverter: ErrorBodyConverter) {
        self.tokenRefresher = tokenRefresher
        self.session = sessionProvider.session
        self.errorBodyConverter = errorBodyConverter
    }
    ...
    private func singleFromRequest<D>(_ request: NetworkRequest) -> Single<D> where D : Decodable {
        return Single<D>.create { observer -> Disposable in
            ...
            r.processWithObserver(observer, converter: self.errorBodyConverter)
            return Disposables.create {
                _ = r.cancel()
            }
        }
...
fileprivate extension DataRequest {
    ...
    func processWithObserver<D>(_ observer: @escaping (SingleEvent<D>) -> Void, converter: ErrorBodyConverter) where D: Decodable {
        self.responseDecodable(of: D.self) {
            if let error = $0.validate(converter) {
                observer(.error(error))
                return
            }
    ...
    func processWithObserver(_ observer: @escaping (CompletableEvent) -> Void, converter: ErrorBodyConverter) {
        self.response {
            if let error = $0.validate(converter) {
                observer(.error(error))
                return
    ...
    func processWithObserver(_ observer: AnyObserver<Progress>, converter: ErrorBodyConverter) {
    ...
    func processWithObserver<D>(_ observer: AnyObserver<ProgressWithResult<D>>, converter: ErrorBodyConverter) where D: Decodable {
        downloadProgress(closure: {
...
extension DataResponse {
    ...
    func validate(_ converter: ErrorBodyConverter) -> Error? {
        ...
        return converter.convert(data, from: response)
    }
